...
public sealed class Forge : IXModLoader
{
    private const string MavenBase = "https://maven.minecraftforge.net";
    ...
    public async Task SetupAsync(CancellationToken ct = default)
    {
        ...
        _log.Info($"Forge 执行安装: {installCmd}");
    ...
    {
        var rawUrl = $"{MavenBase}/net/minecraftforge/forge/{_mcv}-{_mlv}/forge-{_mcv}-{_mlv}-installer.jar";
        ...
        if (mirrorUrl != rawUrl)
        {
            try
            {
                _log.Info($"下载 Forge installer (镜像): {mirrorUrl}");
        ...
        _log.Info($"下载 Forge installer: {rawUrl}");
        await _downloadService.DownloadFileAsync(rawUrl, filePath, ct: ct).ConfigureAwait(false);
    }

    private string GetInstallerJarName() => $"forge-{_mcv}-{_mlv}-installer.jar";
}
